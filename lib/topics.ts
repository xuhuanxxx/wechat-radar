...
const PLACEHOLDER_PATTERNS = [
  /^\[链接\]\s*当前(?:客户端)?版本不支持/,
  /^当前(?:客户端)?版本不支持展示该内容，请升级至?最新(?:版|版本)[。.]?$/,
  /^\[文件\]\s*[^\s]+\.\w+\s*$/,
  /^\[视频\]\s*$/,
  /^\[音频\]\s*$/,
  /^\[语音\]\s*$/,
...
function isPlaceholderTitle(value: string): boolean {
  const text = value
    ...
    .trim();
  if (!text) return true;
  return /^(?:当前(?:客户端)?版本不支持展示该内容，请升级至?最新(?:版|版本)|当前版本不支持|请升级至最新版本)[。.]?$/i.test(text);
}
...
function buildExtractionPrompt(
  ...
  return `你是群聊「话题雷达」的聚合引擎。请直接用 LLM 判断语义相关性，找出 ${date} 的主要讨论话题。
...
- 严禁把「当前版本不支持展示该内容」「当前客户端版本不支持展示该内容」「请升级至最新版本」这类占位文案当作话题标题或摘要。
...
- 删除过泛、重复、证据不足的话题。
- 删除客户端升级提示、资源封面、头像链接、无语义数字串等占位话题。
